(ns eleclighting.sim
  ...
  proposal that tries to self-issue an electric-lighting-equipment
  safety-certification mark (permanently blocked, no override), a
  ...
  an implausible dielectric-test-kv reading, and a production-batch
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t1"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-type :led-lamp :last-assessed "2026-07-14"}}
                       coordinator))

    (println "== schedule-maintenance mnt-1 on led-assy-001 (verified, registered, LED-assembly line -- escalates, approve) ==")
    ...
    (println "== flag-safety-concern concern-1 on led-assy-001 (always escalates -- approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on photometric-test-002 (UNVERIFIED/unregistered test bench -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on led-assy-001 with :actuate-equipment? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 attempting to self-issue an electric-lighting-equipment safety certification -> HARD hold, PERMANENT ==")
