(ns eleclighting.operation
  "ElecLightingOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (ElecLightingAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Electric Lighting Plant Operations Governor (:govern) and the
...
(defn build
  "Compiles an ElecLightingOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `eleclighting.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ElecLightingAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Electric Lighting Plant Operations Governor -- independent censor.
      (g/add-node :govern
