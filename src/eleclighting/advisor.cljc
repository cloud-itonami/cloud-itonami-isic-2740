(ns eleclighting.advisor
  "ElecLightingAdvisor -- the *contained intelligence node* for the
  electric-lighting-equipment plant-operations coordination actor.

  It normalizes production-batch patches (product-type/dielectric-
  test-kv/quantity/defect-rate), drafts an assembly/test-bench-
  ...
  electric-lighting-equipment product shipment coordination proposal
  ...
  cited), never a committed record and NEVER a real assembly/test-
  bench-equipment actuation, freight dispatch, or safety-
  certification -- see README `What this actor does NOT do`. Every
  output is censored downstream by `eleclighting.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `eleclighting.governor`)
  ...
  contract every caller of this actor agrees to) -- `eleclighting.governor`
...
(defn- schedule-maintenance
  ...
  rationale, but `eleclighting.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft an electrical-safety/photobiological-hazard/UL-CE-compliance
  ...
  technicality). See `eleclighting.phase`: no phase ever adds this op
  to a phase's `:auto` set; `eleclighting.governor` also always
...
(defn- coordinate-shipment
  "Draft an outbound electric-lighting-equipment product shipment
  coordination proposal against a production batch. The advisor
  passes through the caller's own claimed unit quantity -- it does NOT
  invent one, and `eleclighting.governor` NEVER trusts it: it
  independently recomputes whether the batch's own cumulative-shipped
  quantity plus this claim would exceed the batch's own recorded
  quantity before any commit is possible."
  [db {:keys [subject value]}]
...
(def ^:private system-prompt
  (str "あなたはLED照明機器(LEDランプ・LED照明器具・LEDドライバー(制御装置)・"
       "ダウンライト・街路灯)製造工場"
       ...
       "電気照明機器安全認証(UL/CE等)を自己発行する提案をしてはいけません。"
       "出荷数量を偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `eleclighting.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :eleclighting-advisor-proposal
   :op         (:op request)
