(ns eleclighting.store
  "SSoT for the electric-lighting-equipment plant-operations
  ...
  Scope note: like its siblings (`cloud-itonami-isic-2750`'s own
  `domappl.store`, `cloud-itonami-isic-2710`'s own
  ...
  is self-contained (no external electric-lighting-manufacturing
  ...
  Four kinds of entity live here:
    ...
    - `maintenance`       -- a scheduled assembly/test-bench
                             ...
                             of equipment (`eleclighting.registry`'s
    ...
    - `shipments`         -- a proposed outbound electric-lighting-
                             equipment product shipment DRAFT
                             (`eleclighting.registry`'s
                             `register-shipment`).
  ...
  coordinator needs."
  (:require [eleclighting.registry :as registry]))
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (eleclighting.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (eleclighting.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :led-lamp
                :model "LED-A19-9W" :dielectric-test-kv 2.0
                :quantity-units 5000.0 :defect-rate-percent 0.6
                :verified? true :registered? true
                :shipped-units 1000.0
                :last-assessed "2026-07-01"}
   "batch-002" {:id "batch-002" :product-type :led-luminaire
                :model "LUM-600x600" :dielectric-test-kv 3.0
                :quantity-units 800.0 :defect-rate-percent 0.9
                :verified? true :registered? true
                :shipped-units 750.0
                :last-assessed "2026-07-01"}
   "batch-003" {:id "batch-003" :product-type :street-light-fixture
                :model "SL-100W" :dielectric-test-kv 1.0
                :quantity-units 300.0 :defect-rate-percent 1.2
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-06-15"}})

(defn- sample-equipment []
  {"led-assy-001" {:id "led-assy-001" :kind :led-assembly-line
                    :verified? true :registered? true
                    :last-maintenance-date "2026-06-01"}
   "photometric-test-002" {:id "photometric-test-002" :kind :photometric-test-bench
                            :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `eleclighting.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `eleclighting.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  LED-assembly-line unit (schedulable for maintenance), one
  UNVERIFIED/unregistered photometric-test-bench unit (blocks any
