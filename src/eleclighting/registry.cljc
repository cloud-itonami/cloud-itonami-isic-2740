(ns eleclighting.registry
  "Pure-function domain logic for the electric-lighting-equipment
  ...
  pre-existing `kotoba-lang/eleclighting`-style capability library to
  wrap (verified: no such repo exists). The domain logic therefore
  lives here as pure functions, re-verified INDEPENDENTLY by
  `eleclighting.governor` -- the same 'ground truth, not self-report'
  discipline every sibling actor's own registry establishes (e.g.
  `domappl.registry/shipment-quantity-exceeded?` from
  `cloud-itonami-isic-2750`, and `elecequipmfg.registry` from
  ...
  of issuing an electric-lighting-equipment safety-certification mark
  (e.g. UL/CE) (this actor NEVER does any of those -- see README `What
  this actor does NOT do`).

  SCOPE: ISIC 2740 covers manufacture of electric lighting equipment --
  LED/lamp-assembly, fixture-housing/optics-assembly, and photometric/
  electrical test lines producing finished LED lamps, LED luminaires,
  LED drivers (control gear), downlight fixtures, and street-light
  fixtures. This actor coordinates the back-office record-keeping
  ...
  the assembly/test-bench equipment directly, and it never stands in
  for the certification body that issues electric-lighting-equipment
  safety/UL/CE compliance marks.")
...
(def valid-product-types
  ...
  through."
  #{:led-lamp :led-luminaire :led-driver :downlight-fixture :street-light-fixture})
...
(def dielectric-test-kv-max
  ...
  safety-test voltage reading, in kV. Grounded in IEC 60598-1's
  (luminaires -- general requirements and tests) and IEC 61347-1's
  (lamp control gear -- general and safety requirements) electric-
  strength (hipot) test-voltage tables, which top out around 4 kV for
  double/reinforced-insulation (Class II) mains-connected lighting
  equipment -- a reading above this is implausible sensor/QC data, not
  a real routine dielectric test on any standardized class of electric
  lighting equipment."
  4.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own assembly/test defect-rate reading
  (zero defects is the best possible outcome, never negative)."
  0.0)

(def defect-rate-max-percent
  "Physical ceiling for a batch's own assembly/test defect-rate
...
(defn defect-rate-valid?
  "Is `percent` a physically plausible batch assembly/test defect-rate
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER an electric-lighting-equipment safety-
  certification mark -- this actor is never the certification
...
(defn register-maintenance
  ...
  the RECORD a plant coordinator would keep. `eleclighting.governor`
...
(defn register-shipment
  ...
  outbound electric-lighting-equipment product shipment against a
  ...
  coordinator would keep. `eleclighting.governor` independently
