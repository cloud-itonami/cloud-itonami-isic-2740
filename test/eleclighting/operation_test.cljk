(ns eleclighting.operation-test
  "Smoke tests for the compiled ElecLightingOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `eleclighting.governor-contract-test`; the Store contract in
  `eleclighting.store-contract-test`."
...
(deftest test-actor-builds
  (testing "ElecLightingOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "led-assy-001" :severity :moderate :description "insulation-resistance anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
