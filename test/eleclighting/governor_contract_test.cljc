(ns eleclighting.governor-contract-test
  ...
  operations... does NOT self-issue an electric-lighting-equipment
  safety-certification mark') implemented faithfully. The single
  invariant under test:

    ElecLightingAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Electric Lighting Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :led-lamp}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :led-lamp (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the electric-lighting-equipment safety-certification authority"
    (let [[db actor] (fresh)
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "led-assy-001" :severity :moderate
                                            :description "insulation-resistance anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "led-assy-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
